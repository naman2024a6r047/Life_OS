...
let svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 163 393.75" fill-rule="evenodd" clip-rule="evenodd">\n`;
svg += `  <style>.muscle path { stroke: #27272a; stroke-width: 1.5; }</style>\n`;

for (const [groupName, paths] of Object.entries(map)) {
  svg += `  <g id="${groupName}" class="muscle" data-muscle="${groupName}" fill="#52525b">\n`;
  if (paths.length > 0) {
    paths.forEach((d, i) => {
...
if (trashPaths.length > 0) {
  svg += `  <g id="unmapped_decorative" fill="#52525b" opacity="1">\n`;
...
console.log('Successfully regenerated anatomy.svg with 163 393.75 viewBox and higher visibility!');
